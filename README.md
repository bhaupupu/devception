...
The backend can be deployed to Render as a Docker web service. The Docker image installs Node.js dependencies plus `python3` and `g++`, which are required by the in-game code runner.

### Option A: Render Blueprint

1. Push this repository to GitHub.
2. In Render, choose **New +** -> **Blueprint**.
3. Select the repository and use the root [render.yaml](render.yaml).
4. Fill the secret environment variables Render asks for:

```env
MONGODB_URI=your-public-mongodb-uri
NEXTAUTH_SECRET=the-same-secret-used-by-vercel-nextauth
...
Important: Railway private MongoDB URLs such as `mongodb.railway.internal` only work from Railway services. Render needs a public MongoDB connection string, for example MongoDB Atlas, a Railway public TCP proxy URL, or a migrated database.
...
```text
Runtime: Docker
Root directory: devception-server
...
After Render deploys successfully, update the Vercel frontend environment variables:
...
For the configured Render service name in this repo, the expected values are:
...
```env
NEXTAUTH_SECRET=the-same-secret-used-by-render
...
- Log in on the Vercel frontend.
- Confirm the dashboard/profile API calls work.
- Create a private room.
- Join a lobby and confirm Socket.IO connects.
- Start a game and verify editor sync, chat, tasks, meetings, and game results.
- Run a JavaScript task.
- Run Python/C++ tasks if those languages are enabled in the UI.

### Rollback

Do not delete the Railway service until Render is verified. To roll back, restore the old Vercel values:
...
Then redeploy Vercel. The old [devception-server/railway.json](devception-server/railway.json) remains in the repo for rollback.
