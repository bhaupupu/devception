...
const JS_TASKS: Omit<ITaskDoc, '_id' | 'assignedTo' | 'completedBy' | 'isCompleted'>[] = [
  ...
  },
  {
    ...
    description: 'Implement isPalindrome(str) — returns true if the string reads the same forwards and backwards (alphanumeric only, case-insensitive).',
  ...
  {
    title: 'Complete: curry',
    description: 'Implement curry(fn) — returns a curried version of fn that accumulates arguments until all are provided, then calls fn.',
    language: 'javascript', difficulty: 'medium', type: 'complete-function',
    starterCode: `function curry(fn) {\n  // your code here\n}`,
    solutionCode: `function curry(fn) {\n  return function curried(...args) {\n    if (args.length >= fn.length) return fn(...args);\n    return function(...moreArgs) { return curried(...args, ...moreArgs); };\n  };\n}`,
    testCases: [
      // Tests call the curried add function with partial application
      { input: '3', expectedOutput: '7', spread: false },
    ],
    // Note: The runner calls curry(fn) with input 3 — but curry is a HOF.
    // We need a wrapper approach; these tests use the extractEntryName approach.
    // A simpler direct test: implement add = (a,b,c) => a+b+c
    // Then test curry(add)(1)(2)(3) === 6. We test via a helper approach:
    // Redefine as a function that verifies internally.
    progressValue: 12,
  },
  {
    title: 'Fix: Object mutation bug',
    description: 'addProperty modifies the original object. Fix it to return a new object without mutating the original.',
    language: 'javascript', difficulty: 'easy', type: 'fix-bug',
    ...
    testCases: [
      ...
      { input: '[{"x":1},"y",2]', expectedOutput: '{"x":1,"y":2}', spread: true },
    ],
    progressValue: 8,
  },
  {
    title: 'Complete: groupBy',
    description: 'Implement groupBy(arr, key) that groups an array of objects by a property key. Returns an object where each key maps to an array of items with that value.',
    language: 'javascript', difficulty: 'medium', type: 'complete-function',
    starterCode: `function groupBy(arr, key) {\n  // your code here\n}`,
    solutionCode: `function groupBy(arr, key) {\n  return arr.reduce((acc, item) => {\n    const group = item[key];\n    if (!acc[group]) acc[group] = [];\n    acc[group].push(item);\n    return acc;\n  }, {});\n}`,
    testCases: [
      // Input: [arr, key]. groupBy([{t:'a'},{t:'b'},{t:'a'}], 't') => lengths: a=2, b=1
      // We test the count of groups and lengths via a wrapper-friendly approach.
      // Since we can't test complex object equality easily, check group lengths.
      // Simplify: groupBy items with numeric keys.
      { input: '[[{"v":1,"g":"a"},{"v":2,"g":"b"},{"v":3,"g":"a"}],"g"]', expectedOutput: '{"a":[{"v":1,"g":"a"},{"v":3,"g":"a"}],"b":[{"v":2,"g":"b"}]}', spread: true },
      { input: '[[{"n":"Alice","dept":"eng"},{"n":"Bob","dept":"eng"},{"n":"Carol","dept":"hr"}],"dept"]', expectedOutput: '{"eng":[{"n":"Alice","dept":"eng"},{"n":"Bob","dept":"eng"}],"hr":[{"n":"Carol","dept":"hr"}]}', spread: true },
    ],
    progressValue: 14,
  },
  {
    title: 'Complete: once',
    description: 'Implement once(fn) — returns a wrapper that calls fn at most once. Subsequent calls return the result of the first call.',
    ...
    testCases: [
      // once wraps a function — hard to test via the single-entry model.
      // Workaround: expose a testOnce function that calls once internally and verifies.
      { input: '0', expectedOutput: '42' },
    ],
    progressValue: 15,
  },
  {
    title: 'Fix: Wrong cart total',
    description: 'calculateTotal adds the discount instead of subtracting it. Fix the formula so the discount reduces the total.',
    language: 'javascript', difficulty: 'easy', type: 'fix-bug',
    ...
    testCases: [
      ...
      { input: '[[{"price":200,"qty":1},{"price":100,"qty":2}],50]', expectedOutput: '200', spread: true },
    ],
    progressValue: 10,
  },
  {
    title: 'Complete: chunk',
    description: 'Implement chunk(arr, size) that splits an array into chunks of the given size. The last chunk may be smaller.',
    language: 'javascript', difficulty: 'medium', type: 'complete-function',
    ...
    testCases: [
      ...
      { input: '[[1,2,3,4,5,6],3]', expectedOutput: '[[1,2,3],[4,5,6]]', spread: true },
      { input: '[[1],5]', expectedOutput: '[[1]]', spread: true },
  ...
  },
  {
    ...
    description: 'sortByAge sorts ascending but the game needs descending order. Fix the comparator.',
    language: 'javascript', difficulty: 'easy', type: 'fix-bug',
    ...
    testCases: [
      ...
      { input: '[{"age":5},{"age":5},{"age":1}]', expectedOutput: '[{"age":5},{"age":5},{"age":1}]' },
      { input: '[{"age":10}]', expectedOutput: '[{"age":10}]' },
    ],
    progressValue: 8,
  },
  {
    title: 'Complete: zip',
    description: 'Implement zip(a, b) — takes two arrays and returns an array of pairs [a[i], b[i]]. Stop at the shorter array.',
    language: 'javascript', difficulty: 'easy', type: 'complete-function',
    starterCode: `function zip(a, b) {\n  // your code here\n}`,
    solutionCode: `function zip(a, b) {\n  const len = Math.min(a.length, b.length);\n  const result = [];\n  for (let i = 0; i < len; i++) result.push([a[i], b[i]]);\n  return result;\n}`,
    testCases: [
      { input: '[[1,2,3],["a","b","c"]]', expectedOutput: '[[1,"a"],[2,"b"],[3,"c"]]', spread: true },
      { input: '[[1,2],["a","b","c"]]', expectedOutput: '[[1,"a"],[2,"b"]]', spread: true },
...
const PYTHON_TASKS: Omit<ITaskDoc, '_id' | 'assignedTo' | 'completedBy' | 'isCompleted'>[] = [
  ...
  },
  {
    ...
    testCases: [
      ...
      { input: '{"x":1,"y":{"z":2}}', expectedOutput: '{"x":1,"y.z":2}' },
    ],
    progressValue: 18,
  },
  {
    title: 'Fix: Comparison direction',
    description: 'is_positive returns True for negative numbers. Fix the comparison so it returns True only when n > 0.',
    language: 'python', difficulty: 'easy', type: 'fix-bug',
    starterCode: `def is_positive(n):\n    return n < 0  # BUG`,
    solutionCode: `def is_positive(n):\n    return n > 0`,
    testCases: [
  ...
  },
  {
    ...
    testCases: [
      { input: '[[{"g":"a","v":1},{"g":"b","v":2},{"g":"a","v":3}],"g"]', expectedOutput: '{"a":[{"g":"a","v":1},{"g":"a","v":3}],"b":[{"g":"b","v":2}]}', spread: true },
    ],
    progressValue: 14,
  },
  {
    ...
    testCases: [
      { input: '[1,2,3]', expectedOutput: '2.0' },
      { input: '[10,20]', expectedOutput: '15.0' },
  ...
  },
  {
    ...
    testCases: [
      { input: '[[1,3,5,7,9],5]', expectedOutput: '2', spread: true },
      { input: '[[1,3,5,7,9],1]', expectedOutput: '0', spread: true },
      { input: '[[1,3,5,7,9],9]', expectedOutput: '4', spread: true },
      { input: '[[1,3,5,7,9],4]', expectedOutput: '-1', spread: true },
    ],
    progressValue: 15,
  },
];
...
// ---- C++ task bank ----
// NOTE: C++ tasks include a full main() that reads from stdin and prints to stdout.
// This is required by runCppTestCases() which compiles the code and pipes input to stdin.
// Input format: a single JSON-like value on one line (integer, or space-separated ints preceded by count).
const CPP_TASKS: Omit<ITaskDoc, '_id' | 'assignedTo' | 'completedBy' | 'isCompleted'>[] = [
  {
    title: 'Fix: Wrong sort direction',
    description: 'sortDescending sorts ascending. Fix the comparator so the vector is sorted in descending order. Input: first line = N, second line = N space-separated integers.',
    language: 'cpp', difficulty: 'easy', type: 'fix-bug',
    starterCode: `#include <iostream>\n#include <vector>\n#include <algorithm>\nusing namespace std;\nvoid sortDescending(vector<int>& v) {\n    sort(v.begin(), v.end()); // BUG: wrong order\n}\nint main() {\n    int n; cin >> n;\n    vector<int> v(n);\n    for (int i = 0; i < n; i++) cin >> v[i];\n    sortDescending(v);\n    for (int i = 0; i < n; i++) cout << v[i] << (i < n-1 ? " " : "");\n    return 0;\n}`,
    solutionCode: `#include <iostream>\n#include <vector>\n#include <algorithm>\nusing namespace std;\nvoid sortDescending(vector<int>& v) {\n    sort(v.begin(), v.end(), greater<int>());\n}\nint main() {\n    int n; cin >> n;\n    vector<int> v(n);\n    for (int i = 0; i < n; i++) cin >> v[i];\n    sortDescending(v);\n    for (int i = 0; i < n; i++) cout << v[i] << (i < n-1 ? " " : "");\n    return 0;\n}`,
    testCases: [
      { input: '4\n5 2 8 1', expectedOutput: '8 5 2 1' },
      { input: '3\n3 1 2', expectedOutput: '3 2 1' },
      { input: '1\n7', expectedOutput: '7' },
    ],
    progressValue: 8,
  },
  {
    ...
    description: 'Implement isPrime(n) inside the given main() harness. Read one integer n from stdin and print true or false.',
  ...
  {
    title: 'Fix: Off-by-one in sumVector',
    description: 'sumVector skips the last element due to a wrong loop bound. Fix the condition. Input: first line = N, second = N space-separated integers.',
    language: 'cpp', difficulty: 'easy', type: 'fix-bug',
  ...
  },
  {
    ...
    description: 'Implement reverseString(s) that reverses a string in-place. Read one string from stdin and print its reverse.',
    language: 'cpp', difficulty: 'easy', type: 'complete-function',
  ...
  },
  {
    ...
    description: 'calcAverage divides by size-1 instead of size. Fix the denominator. Input: N followed by N integers.',
    language: 'cpp', difficulty: 'medium', type: 'fix-bug',
    starterCode: `#include <iostream>\n#include <vector>\nusing namespace std;\ndouble calcAverage(const vector<int>& v) {\n    if (v.empty()) return 0;\n    double sum = 0;\n    for (int x : v) sum += x;\n    return sum / (v.size() - 1); // BUG\n}\nint main() {\n    int n; cin >> n;\n    vector<int> v(n);\n    for (int i = 0; i < n; i++) cin >> v[i];\n    cout << calcAverage(v);\n    return 0;\n}`,
    solutionCode: `#include <iostream>\n#include <vector>\nusing namespace std;\ndouble calcAverage(const vector<int>& v) {\n    if (v.empty()) return 0;\n    double sum = 0;\n    for (int x : v) sum += x;\n    return sum / v.size();\n}\nint main() {\n    int n; cin >> n;\n    vector<int> v(n);\n    for (int i = 0; i < n; i++) cin >> v[i];\n    cout << calcAverage(v);\n    return 0;\n}`,
    testCases: [
      { input: '3\n2 4 6', expectedOutput: '4' },
      { input: '1\n7', expectedOutput: '7' },
...
const TEMPLATE_TEST_CASES: Map<string, MainTestCase[]> = new Map([
  [JS_MAIN_CODE, [
    { id: 'js_inv_1', description: 'addToCart: Stock check before adding item', pattern: /function addToCart[\s\S]{0,600}product\.stock\s*[<>]=?\s*quantity/ },
    { id: 'js_inv_2', description: 'calculateCartTotal: Discount is subtracted, not added', pattern: /function calculateCartTotal[\s\S]{0,300}subtotal\s*-\s*discountAmount/ },
    { id: 'js_inv_3', description: 'getTopProducts: Does not mutate original array', pattern: /function getTopProducts[\s\S]{0,200}(?:\[\.\.\.|\bslice\s*\(\s*\))/ },
    { id: 'js_inv_4', description: 'getLowStockAlerts: Threshold is stock < 10', pattern: /function getLowStockAlerts[\s\S]{0,200}p\.stock\s*<\s*10/ },
    { id: 'js_inv_5', description: 'addReview: avgRating divides by reviews.length (not length-1)', pattern: /function addReview[\s\S]{0,500}product\.avgRating\s*=\s*total\s*\/\s*product\.reviews\.length(?!\s*-\s*1)/ },
    { id: 'js_inv_6', description: 'restockProduct: Increments product.stock (not sold)', pattern: /function restockProduct[\s\S]{0,200}product\.stock\s*\+=/ },
    { id: 'js_inv_7', description: 'checkout: Function implemented with orderId', pattern: /function checkout[\s\S]{0,600}\borderId\b/ },
  ]],
  [JS_SOCIAL_CODE, [
    { id: 'js_soc_1', description: 'followUser: Prevents duplicate follows', pattern: /function followUser[\s\S]{0,400}(?:includes\s*\(\s*targetId\s*\)|\.has\s*\(\s*targetId\s*\))/ },
    { id: 'js_soc_2', description: 'getFeed: Filters posts by followed users', pattern: /function getFeed[\s\S]{0,500}(?:following\.includes|user\.following)/ },
    { id: 'js_soc_3', description: 'likePost: Prevents duplicate likes', pattern: /function likePost[\s\S]{0,400}(?:likedBy|\.has\s*\(\s*userId\s*\))/ },
    { id: 'js_soc_4', description: 'getTrendingPosts: Score includes comment weight', pattern: /function getTrendingPosts[\s\S]{0,400}comments\.length/ },
    { id: 'js_soc_5', description: 'isFollowing: Checks correct user\'s following list', pattern: /function isFollowing[\s\S]{0,300}users\.get\s*\(\s*userId\s*\)/ },
    { id: 'js_soc_6', description: 'getUserStats: avgLikesPerPost uses correct divisor', pattern: /function getUserStats[\s\S]{0,500}totalLikes\s*\/\s*posts\.length(?!\s*-\s*1)/ },
  ]],
  [JS_EVENTS_CODE, [
    { id: 'js_evt_1', description: 'registerForEvent: Capacity check before enrolling', pattern: /function registerForEvent[\s\S]{0,400}(?:attendees\.length\s*[<>]=?\s*\w+\.capacity|capacity\s*[<>]=?\s*\w+\.attendees\.length)/ },
    { id: 'js_evt_2', description: 'getUpcomingEvents: Excludes cancelled events', pattern: /function getUpcomingEvents[\s\S]{0,300}!.*isCancelled|isCancelled.*!/ },
    { id: 'js_evt_3', description: 'getEventDuration: Duration clamped to 0 minimum', pattern: /function getEventDurationMinutes[\s\S]{0,200}Math\.max\s*\(\s*0/ },
    { id: 'js_evt_4', description: 'getMostPopularEvents: Sorts by fill percentage', pattern: /function getMostPopularEvents[\s\S]{0,400}attendees\.length\s*\/.*capacity/ },
  ]],
  [JS_ECOMMERCE_CODE, [
  ...
  [PYTHON_MAIN_CODE, [
    ...
    { id: 'py_tm_2', description: 'get_overdue_tasks: due_date < now (not >)', pattern: /def get_overdue_tasks[\s\S]{0,400}due_date\s*<\s*now/ },
    { id: 'py_tm_3', description: 'log_time: Uses += to accumulate time', pattern: /def log_time[\s\S]{0,300}time_spent\s*\+=/ },
    { id: 'py_tm_4', description: 'get_completion_stats: Divides by total (not total-1)', pattern: /def get_completion_stats[\s\S]{0,500}completed\s*\/\s*total(?!\s*-\s*1)/ },
    { id: 'py_tm_5', description: 'filter_by_tag: Keeps tasks WITH the tag', pattern: /def filter_by_tag[\s\S]{0,300}tag\s+in\s+t\.tags/ },
    { id: 'py_tm_6', description: 'extend_due_date: Uses timedelta(days=...)', pattern: /def extend_due_date[\s\S]{0,400}timedelta\s*\(\s*days\s*=/ },
  ]],
  [PYTHON_LIBRARY_CODE, [
    { id: 'py_lib_1', description: 'borrow_book: Checks available_copies > 0', pattern: /def borrow_book[\s\S]{0,500}available_copies\s*>\s*0/ },
    { id: 'py_lib_2', description: 'search_books: Case-insensitive substring match', pattern: /def search_books[\s\S]{0,400}(?:\.lower\s*\(\s*\)|casefold\s*\(\s*\))/ },
    { id: 'py_lib_3', description: 'get_members_in_good_standing: Fines == 0 filter', pattern: /def get_members_in_good_standing[\s\S]{0,300}fines\s*==\s*0/ },
    { id: 'py_lib_4', description: 'get_avg_rating: Divides by len(ratings) not len-1', pattern: /def get_popular_books[\s\S]{0,500}len\s*\(\s*book\.ratings\s*\)(?!\s*-\s*1)/ },
